...
class App extends Component {
  ...
  getFile() {
    ...
    fetch(FOLDER + `${formatLocation}.csv`)
      ...
      .then(() =>
        this.setState({
          timer: setInterval(() => {
            fetch(FOLDER + `${formatLocation}.csv`).then(response =>
                    ...
                    () => {
                      if (
                        ...
                        this.setState({
                          ...
                          availableOpens: parseInt(
                            this.state.opens.match(/\d+/)[0]
                          )
                        })
                      }
                      this.filter()
    ...
    if (this.state.leftClick) {
      this.setState({
        selectedTd: this.state.selectedTd
          ? this.state.selectedTd[0] === rowKeyValue &&
            this.state.selectedTd[1] === objectKeyIndex
            ? []
            : [rowKeyValue, objectKeyIndex]
          : [rowKeyValue, objectKeyIndex]
      })
    }
  }
  ...
  render() {
    ...
    const shiftOneFlights = shiftOne ? parseInt(shiftOne[1].match(/\d+/)[0]) : 0
    const shiftTwoFlights = shiftTwo ? parseInt(shiftTwo[1].match(/\d+/)[0]) : 0
